...
type CityMedia = {
  ...
  // for now just colors/gradients as placeholders; later swap to real image URLs
};
...
export default function CityArchivePage() {
  ...
  const city = formatCitySlug(params.city || "city");

  const [filter, setFilter] = useState<TimeFilter>("all");
  const [nearestFirst, setNearestFirst] = useState(true);

  // stub items (replace later with real media)
  const items = useMemo<CityMedia[]>(
    () => [
  ...
  // fake filtering (just to wire UI)
  const filtered = useMemo(() => {
    // later: real grouping by year/month/day
    let out = items;

    if (filter === "yearly") out = items.slice(0, 4);
    if (filter === "monthly") out = items.slice(1, 5);
    if (filter === "daily") out = items.slice(2, 6);

    const sorted = [...out].sort((a, b) => {
      if (nearestFirst) return b.dateISO.localeCompare(a.dateISO); // newest first
      return a.dateISO.localeCompare(b.dateISO); // oldest first
    });

    return sorted;
  }, [items, filter, nearestFirst]);

  return (
    <div className="min-h-screen w-full bg-black relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 z-0">
            <img
            src="/bg/archive_city.png"
            alt=""
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        <div className="flex-1 px-6 pt-10 pb-[92px]">
          ...
          <div className={[dotGothic.className, "text-white text-[44px] leading-[0.95]"].join(" ")}>
            <div>archives on:</div>
            <div className="text-purple-200">{city}</div>
          </div>
              ...
              <div className="rounded-2xl border border-white/15 bg-black/70 backdrop-blur-md p-2">
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className={`${spaceMono.className} flex-1 h-10 rounded-xl border border-white/40 bg-black/10 text-[12px] text-white/85 px-3`}
                  >
